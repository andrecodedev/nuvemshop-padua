...
{% endif %}
        ...
        <form class="js-product-form" method="post" action="{{ store.cart_url }}">
            ...
            <div class="item-description py-2{% if settings.theme_rounded %} px-3{% else %} px-1{% endif %}" data-store="product-item-info-{{ product.id }}">
                ...
                {% if settings.product_color_variants %}
                    {% include 'snipplets/grid/item-colors.tpl' with {quickshop: true}  %}
                {% endif %}
        ...
        {% if settings.quick_shop and has_buttons %}
            <div class="item-actions row justify-content-center{% if settings.theme_rounded %} m-0 mb-3{% endif %}">
                {% if product.available and product.display_price %}
                    <div class="col-8 col-md-6 pr-1">
                        {% if product.variations %}
                            <a href="#" class="js-item-buy-open item-buy-open d-block btn btn-primary btn-small" title="{{ 'Compra rápida de' | translate }} {{ product.name }}" aria-label="{{ 'Compra rápida de' | translate }} {{ product.name }}" data-component="product-list-item.add-to-cart" data-component-value="{{product.id}}">SHOP NOW</a>
                        {% else %}
                            <form class="js-product-form" method="post" action="{{ store.cart_url }}">
                                <input type="hidden" name="add_to_cart" value="{{product.id}}" />
                                {% set state = store.is_catalog ? 'catalog' : (product.available ? product.display_price ? 'cart' : 'contact' : 'nostock') %}
