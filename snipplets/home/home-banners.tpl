...
<div class="js-category-banner-fullwidth container{% if settings.banners_full %}-fluid{% endif %}">
    <div class="row">
        ...
        {% if use_defaults %}
            {% for i in [1,2,3] %}
                <div class="js-category-banner-container js-category-banner-container-{{ i }} col-md-4" data-visible="false">
                    <div class="textbanner{% if settings.theme_rounded %} box-rounded textbanner-shadow{% endif %}">
                        <div class="textbanner-image">
                            <img class="textbanner-image-background" src="https://placehold.co/620x715?text=+620x715 : Banners+de+categorias" alt="{{ 'Placeholder banner' | translate }} {{ i }}" />
                        </div>
                        <div class="textbanner-text{% if settings.theme_rounded %} textbanner-text-primary{% endif %}">
                            <h3 class="h2 mb-0">{{ 'Categoria' | translate }} {{ i }}</h3>
                            <div class="textbanner-paragraph">{{ 'Explore nossos produtos e coleções' | translate }}</div>
                        </div>
                    </div>
                </div>
            {% endfor %}
