...
<div class="text-left title-section-products">
    <h2 class=" container h1 js-products-{{ highlighted_products_setting_name }}-title " {% if not section_title %}style="display: none"{% endif %}>{{ section_title }}</h2>
</div>
<div class="container home-products-slide">
    <div class="row js-products-{{ highlighted_products_setting_name }}-container" data-format="{{ section_format }}" data-desktop-columns="{% if settings.grid_columns == 2 %}4{% else %}3{% endif %}">
        ...
        {% if use_slider %}
            <div class="js-swiper-{{ highlighted_products_setting_name }}-prev swiper-button-prev d-none d-md-flex svg-circle svg-circle-big svg-icon-text{% if settings.icons_solid %} svg-solid{% endif %}">
                <svg class="icon-inline icon-2x icon-flip-horizontal"><use xlink:href="#chevron"/></svg>
            </div>
            <div class="js-swiper-{{ highlighted_products_setting_name }}-next swiper-button-next d-none d-md-flex svg-circle svg-circle-big svg-icon-text{% if settings.icons_solid %} svg-solid{% endif %}">
