    ...
    <div class="js-product-promo-container mb-4" data-store="product-promotion-info">
        {% if product.promotional_offer.script.is_discount_for_quantity %}
            {% for threshold in product.promotional_offer.parameters %}
                <h4 class="mb-0 text-accent"><strong>{{ "¡{1}% OFF comprando {2} o más!" | translate(threshold.discount_decimal_percentage * 100, threshold.quantity) }}</strong></h4>
            {% endfor %}
        {% else %}
            <h4 class="mb-0 text-accent"><strong>{{ "¡Llevá {1} y pagá {2}!" | translate(product.promotional_offer.script.quantity_to_take, product.promotional_offer.script.quantity_to_pay) }}</strong></h4>
        {% endif %}
        {% if product.promotional_offer.scope_type == 'categories' %}
            <p class="mt-3">{{ "Válido para" | translate }} {{ "este producto y todos los de la categoría" | translate }}:
        ...
        {% elseif product.promotional_offer.scope_type == 'all'  %}
            <p class="mt-3">{{ "Vas a poder aprovechar esta promoción en cualquier producto de la tienda." | translate }}</p>
        {% endif %}
    </div>
{% endif %}
...
{# Product form, includes: Variants, CTA and Shipping calculator #}
 ...
 <form id="product_form" class="js-product-form" method="post" action="{{ store.cart_url }}" data-store="product-form-{{ product.id }}" style="display: flex;flex-direction: column;">
    ...
    <div class="form-row mb-2">
        ...
        {% set texts = {'cart': "SHOP NOW", 'contact': "Consultar precio", 'nostock': "Sin stock", 'catalog': "Consultar"} %}
