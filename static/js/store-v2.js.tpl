...
DOMContentLoaded.addEventOrExecute(() => {
        ...
        if (productDescription) {
            ...
            if (tableStartIndex !== -1) {
                ...
                if (contentBeforeTable) {
                    ...
                    collapsibleContainer.insertAdjacentElement('beforebegin', contentDiv);
                }

                // Agora processa a tabela
                const tableHTML = descriptionHTML.substring(tableStartIndex);
                    ...
                    if (collapsibleContainer) {
                        ...
                        rows.forEach((row, index) => {
                            ...
                            if (cells.length >= 2) {
                                ...
                                collapseTitle.setAttribute('aria-controls', `content-${index}`);
                                collapseTitle.innerText = title;

                                const collapseContent = document.createElement('div');
                                collapseContent.classList.add('collapse-content');
                                collapseContent.setAttribute('id', `content-${index}`);
                                collapseContent.setAttribute('role', 'region');
                                collapseContent.setAttribute('aria-hidden', 'true');
                                collapseContent.innerHTML = content;

                                collapseSection.appendChild(collapseTitle);
                                ...
                                collapseTitle.addEventListener('click', () => {
                                    ...
                                    if (!isVisible) {
                                        ...
                                        collapseTitle.setAttribute('aria-expanded', 'true');
                                    }
                                });
                            }
                        });
  ...
  // Ajusta também para col-lg-6/col-lg-4 e agora grid-4/grid-3 nos banners
  bannerItems.forEach(item => {
    if (columns === 3) {
      // Remove classes de 4 colunas
      item.classList.remove('col-lg-6', 'grid-4');
      // Adiciona classes de 3 colunas
      item.classList.add('col-lg-4', 'grid-3');
    } else if (columns === 4) {
      // Remove classes de 3 colunas
      item.classList.remove('col-lg-4', 'grid-3');
      // Adiciona classes de 4 colunas
      item.classList.add('col-lg-6', 'grid-4');
    }
  });
...
// Mantém os listeners já existentes
document.querySelectorAll('[data-grid]').forEach(button => {
  button.addEventListener('click', function () {
    const gridColumns = parseInt(this.getAttribute('data-grid'));
    changeGrid(gridColumns);
  });
});
...
document.querySelector('[data-grid="3"]').style.opacity = '0.4';
    ...
    */ #}
        ...
        document.querySelectorAll('#filters .form-group').forEach(function (filterPrice) {
            filterPrice.style.display = 'none';
        })
        document.querySelectorAll('#filters .h6').forEach(function (filterHeader) {
            filterHeader.addEventListener('click', function () {
                const filterContent = this.nextElementSibling;
                const isOpen = this.classList.contains('open');

                document.querySelectorAll('#filters .h6').forEach(function (header) {
   ...
   function getQuickShopImgSrc(element){
    ...
    {% if template == 'home' %}
        ...
        createSwiper('.js-swiper-banner_full', {
            ...
            slidesPerView: 1,
            navigation: {
                nextEl: '.js-swiper-banner_full-next',
                prevEl: '.js-swiper-banner_full-prev',
            },
        },function(swiperInstance) {
            window.brandsSwiper = swiperInstance;
        });
